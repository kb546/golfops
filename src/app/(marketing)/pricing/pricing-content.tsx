...
const tiers = [
  {
    ...
    price: "Free",
    period: "",
    description: "Perfect for trying GolfOps with your first event.",
    features: [
      "1 event",
      "Up to 24 players",
      "Event website",
...
const faqs = [
  ...
  {
    question: "Can I try GolfOps for free?",
    answer:
      "Yes! The Starter plan is completely free for your first event with up to 24 players. No credit card required. You only pay when you upgrade to Pro or Premium for more features and capacity.",
  },
  {
          ...
          {tiers.map((tier, i) => (
            <motion.div
              ...
              className={cn(
                ...
                tier.popular
                  ? "border-emerald bg-white shadow-elevated lg:scale-[1.02]"
                  : "border-gray/10 bg-white hover:shadow-card"
              )}
            >
              {tier.popular && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                  <span className="inline-flex whitespace-nowrap rounded-full bg-emerald px-4 py-1.5 text-xs font-bold uppercase tracking-wider text-white">
                    Most Popular
                  </span>
                </div>
              )}
              ...
              <div className="mb-6">
                ...
                <div className="mt-3 flex items-baseline gap-1">
                  <span className="text-4xl font-extrabold text-charcoal font-heading">
                    {tier.price}
                  </span>
                  {tier.period && (
                    <span className="text-base text-gray">{tier.period}</span>
                  )}
                </div>
              ...
              <Link
                ...
                className={cn(
                  ...
                  tier.popular
                    ? "bg-forest text-white hover:bg-forest-light shadow-card"
                    : "border border-gray/20 bg-light text-charcoal hover:bg-gray/10"
                )}
              >
                {tier.cta}
              </Link>
          ...
          <div className="space-y-3">
            {faqs.map((faq, i) => (
              <motion.div
                ...
                className="bg-white rounded-[var(--radius-md)] border border-gray/10 overflow-hidden"
              >
                <button
                ...
                >
                  <span className="text-base font-semibold text-charcoal font-heading pr-4">
                    {faq.question}
                  </span>
                  <HelpCircle
                    className={cn(
                      "w-5 h-5 text-gray flex-shrink-0 transition-transform",
        ...
        <div className="container-marketing text-center">
          ...
          >
            ...
            <p className="text-lg text-gray max-w-xl mx-auto mb-8">
              Your first event is on us. See why organizers love GolfOps.
            </p>
            <Link
              href="/register"
